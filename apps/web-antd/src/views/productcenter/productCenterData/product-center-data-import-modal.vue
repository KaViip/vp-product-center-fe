...
import {
  productCenterDataImportTemplate,
  productCenterDataImport,
} from '#/api/productcenter/productCenterData';
import { useBlobExport } from '#/utils/file/export';
import { validateExcelData, getCellError, OPERATIONAL_TEAM_RULES, type ValidationError } from '../import-validation';
...
const { exportBlob, exportLoading } = useBlobExport(productCenterDataImportTemplate);
...
function renderSheet(sheetName: string) {
  ...
  validationErrors.value = validateExcelData(previewData.value, OPERATIONAL_TEAM_RULES, importMode.value !== 'add data');
}
...
<template>
  <Drawer
    :title="$t('pages.productCenter.importShareClassData')"
    :class="'w-[90%]'"
    :footer="true"
